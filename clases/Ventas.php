...
class ventas{
	...
	public function eliminaVenta($tablainf){
		$c= new conectar();
		$conexion=$c->conexion();

		$sql="DELETE from ventas where id_venta='$tablainf'";
    ...
    // Utiliza consultas preparadas para prevenir la inyección de SQL
    $sql = "SELECT apellido, nombre 
            FROM clientes 
            WHERE id_cliente = ?";
    ...
    if ($stmt) {
        ...
        if ($result) {
            ...
            // Verifica si se obtuvieron resultados válidos
            if ($ver) {
                // Devuelve el nombre completo
                return $ver[0] . " " . $ver[1];
            } else {
                return "Sin cliente (S/C)";
            }
        } else {
